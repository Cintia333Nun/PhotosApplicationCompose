package com.example.photosapplication.data_source

import com.example.photosapplication.R
import com.example.photosapplication.model.Quote

val quotes = arrayOf(
    Quote(
        1,
        "Decisiones",
        "- Desearía que no hubiese ocurrido en mi época -dijo Frodo. - Y yo -respondió Gandalf-, y también todos los que han vivido para ver unos tiempos como estos. Pero no les toca a ellos decidir.\n" + "\n" + "Lo único que debemos decidir es qué hacer con el tiempo que nos es dado.",
        "J.R.R. Tolkien",
        "El Señor de los Anillos",
        R.drawable.imagen1,
        R.drawable.tolkien
    ),
    Quote(
        2,
        "Vivir el presente",
        "Si estás deprimido, vives en el pasado. Si estás inquieto, vives en el futuro. Si estás en paz, vives en el presente.",
        "Lao Tse",
        "",
        R.drawable.imagen2,
        R.drawable.lao_tse
    ),
    Quote(
        3,
        "Camino a la excelencia",
        "Lidiar con la frustración pasajera de no lograr ningún progreso forma parte del camino hacia la excelencia.",
        "Tim Ferris",
        "Armas de titanes",
        R.drawable.imagen3,
        R.drawable.ferris
    ),
    Quote(
        4,
        "Enfocarse en lo positivo",
        "Si quieres hacer un cambio permanente, deja de enfocarte en el tamaño de tus problemas y enfócate en el tamaño de tu ser.",
        "T. Harv Eker",
        "Secretos de la Mente Millonaria",
        R.drawable.imagen4,
        R.drawable.eker
    ),
    Quote(
        5,
        "Dichoso",
        "Así como no es una golondrina o un buen día lo que hace la primavera, tampoco es un buen día o un corto período de tiempo lo que hace a un hombre bendecido y dichoso",
        "Aristoteles",
        "",
        R.drawable.imagen5,
        R.drawable.aristoteles
    ),
    Quote(
        6,
        "Grandes adversidades",
        "Algunas ocasiones tus mayores fortalezas son tus mayores debilidades. Algunas ocasiones de las grandes adversidades, es de donde aprendes mas.",
        "Steve Jobs",
        "Make Something Wonderful",
        R.drawable.imagen6,
        R.drawable.jobs
    ),
    Quote(
        7,
        "Para ser genial",
        "Para ser bueno, debes aprender a ser tú mismo. Y para ser genial, debes sumar habilidades a tus fortalezas, no reemplazarlas.",
        "Chris Voss",
        "Rompe la barrera del no",
        R.drawable.imagen7,
        R.drawable.voss
    ),
    Quote(
        8,
        "Lo que nos sucede",
        "No nos afecta lo que nos sucede, sino lo que nos decimos acerca de lo que nos sucede.",
        "Epicteto",
        "",
        R.drawable.imagen8,
        R.drawable.epicteto
    ),
    Quote(
        9,
        "Significado y propósito",
        "La vida nunca es insoportable por las circunstancias, sino por la falta de significado y propósito.",
        "Victor Frankl",
        "El sentido de la vida",
        R.drawable.imagen9,
        R.drawable.frankl
    ),
    Quote(
        10,
        "Nunca te rindas",
        "Nunca te rindas. ¡Nunca!, ¡nunca!, ¡nunca!. Frente a nada, sea grande o pequeño, enorme o diminuto. Nunca te rindas excepto frente a las convicciones del honor y el buen sentido. Nunca te sometas frente a la fuerza. Nunca te sometas al aparentemente abrumador poder del enemigo.",
        "Winston Churchill",
        "",
        R.drawable.imagen10,
        R.drawable.churchill
    ),
    Quote(
        11,
        "Lo que cuenta",
        "Un éxito nunca es definitivo, un fracaso nunca es fatídico, lo que cuenta es el coraje.",
        "Winston Churchill",
        "",
        R.drawable.imagen11,
        R.drawable.churchill
    ),
    Quote(
        12,
        "Falta de confianza",
        "La falta de confianza mata más sueños que la falta de habilidad.",
        "James Clear",
        "Hábitos Atómicos",
        R.drawable.imagen12,
        R.drawable.clear
    ),
    Quote(
        13,
        "Lo que piensan de nosotros",
        "Los demás no piensan tanto en nosotros como creemos, ni siquiera cuando somos el centro de atención.",
        "Amy Cuddy",
        "El poder de la presencia",
        R.drawable.imagen13,
        R.drawable.cuddy
    ),
    Quote(
        14,
        "Creen en ti",
        "Un individuo seguro -que conoce su identidad y cree en ella, lleva consigo herramientas en lugar de armas. No necesita quedar por encima de los demás. Cuando crees de verdad -en ti, en tus ideas-, no te sientes amenazado, sino seguro.",
        "Amy Cuddy",
        "El poder de la presencia",
        R.drawable.imagen14,
        R.drawable.cuddy
    ),
    Quote(
        15,
        "Fíngelo hasta serlo",
        "No lo finjas hasta conseguirlo, fíngelo hasta serlo.",
        "Amy Cuddy",
        "El poder de la presencia",
        R.drawable.imagen15,
        R.drawable.cuddy
    ),
    Quote(
        16,
        "Hasta el anochecer",
        "Cualquiera puede llevar una carga, por pesada que sea, hasta el anochecer. Cualquiera puede hacer su trabajo, por duro que sea, durante el día. Cualquiera puede llever una vida dulce, paciente, amorosa y pura hasta la puesta del sol. Y esto es todo cuanto la vida significa realmente.",
        "Robert Louis Stevenson",
        "",
        R.drawable.imagen16,
        R.drawable.stevenson
    ),
    Quote(
        17,
        "No rendirse",
        "Estar a la altura de la situación no tenía nada que ver con el talento. Lo más importate para no tirar la toalla era la actitud de <<no rendirse nunca>>.",
        "Amy Cuddy",
        "GRIT",
        R.drawable.imagen17,
        R.drawable.cuddy
    ),
    Quote(
        18,
        "Pasión y perseverancia",
        "Aquello que alcanzamos en la vida depende más de nuestra pasión y perseverancia que de nuestro talento natural.",
        "Angela Duckworth",
        "GRIT",
        R.drawable.imagen18,
        R.drawable.duckworth
    ),
    Quote(
        19,
        "Aprovecha los errores y problemas",
        "Aprovecha los errores y problemas como oportunidades para mejorar y no como razones para desistir.",
        "Angela Duckworth",
        "GRIT",
        R.drawable.imagen19,
        R.drawable.duckworth
    ),
    Quote(
        20,
        "Sufrimiento y desesperanza",
        "No es el sufrimiento lo que lleva a la desesperanza, sino el sufrimiento que creemos no poder controlar.",
        "Angela Duckworth",
        "GRIT",
        R.drawable.imagen20,
        R.drawable.duckworth
    ),
    Quote(
        21,
        "Cualidades",
        "Apunta hacia arriba. Presta atención. Arregla lo que puedas arreglar. No seas arrogante. Esfuérzate por ser humilde. No mientas por nada nunca y sobre todo no te mientas a ti.",
        "Jordan B. Peterson",
        "12 Reglas para vivir",
        R.drawable.imagen21,
        R.drawable.peterson
    ),
    Quote(
        22,
        "Querer a alguien",
        "Cuando quieres a alguien, no es a pesar de sus limitaciones, sino a causa de ellas.",
        "Jordan B. Peterson",
        "12 Reglas para vivir",
        R.drawable.imagen22,
        R.drawable.peterson
    ),
    Quote(
        23,
        "Por que vivir",
        "Aquel que tiene un porqué por el cual vivir, puede soportar cualquier cosa.",
        "Friedrich Nietzche",
        "",
        R.drawable.imagen23,
        R.drawable.nietzsche
    ),
    Quote(
        24,
        "Sacar el máximo provecho",
        "No eres responsable de la mano de cartas que te han repartido. Eres responsable de sacar el máximo de lo que te ha tocado.",
        "Tim Ferris",
        "Armas de titanes",
        R.drawable.imagen24,
        R.drawable.ferris
    ),
    Quote(
        25,
        "Estrategía, suerte y miedo",
        "La esperanza no es una estrategía. La suerte no es un factor. El miedo no es una opción.",
        "Tim Ferris",
        "Armas de titanes",
        R.drawable.imagen25,
        R.drawable.ferris
    ),
    Quote(
        26,
        "No intentes",
        "¡No, no intentes! Hazlo o déjalo. No hay intentos.",
        "Yoda",
        "Star Wars (Episode V)",
        R.drawable.imagen26,
        R.drawable.yoda
    ),
    Quote(
        27,
        "Está bien perder",
        "Está bien perder con el oponente. No perder ante el miedo.",
        "Pat Morita (Señor Miyagi)",
        "The Karate Kid III",
        R.drawable.imagen27,
        R.drawable.morita
    ),
    Quote(
        28,
        "Pequeña decisión",
        "Todas las cosas importantes provienen de comienzos modestos. La semilla de cada hábito es una pequeña decisión.",
        "James Clear",
        "Hábitos atómicos",
        R.drawable.imagen28,
        R.drawable.clear
    ),
    Quote(
        29,
        "Esperanza",
        "Al principio la esperanza es todo lo que tienes.",
        "James Clear",
        "Hábitos atómicos",
        R.drawable.imagen29,
        R.drawable.clear
    ),
    Quote(
        30,
        "Falta de claridad",
        "Muchas veces, lo que parece resistencia es falta de claridad.",
        "Chip Heath & Dan Heath",
        "Cambia el chip",
        R.drawable.imagen30,
        R.drawable.chip
    ),
    Quote(
        31,
        "Pequeños cambios",
        "Los grandes cambios proceden de una sucesión de pequeños cambios.",
        "Chip Heath & Dan Heath",
        "Cambia el chip",
        R.drawable.imagen32,
        R.drawable.chip
    ),
    Quote(
        32,
        "No te detengas",
        "No te detengas cuando estés cansado. Detente cuando hayas terminado.",
        "David Goggins",
        "No me puedes dañar",
        R.drawable.imagen33,
        R.drawable.goggins
    ),
    Quote(
        33,
        "Las cosas malas terminan",
        "Cuando somos abatidos por los dramas de vivir, grandes y pequeños, estamos olvidando que no importa qué tan fuerte sea el dolor, no importa qué tan horrorosa sea la tortura, todas las cosas malas terminan.",
        "David Goggins",
        "No me puedes dañar",
        R.drawable.imagen34,
        R.drawable.goggins
    ),
    Quote(
        34,
        "You’ve never done",
        "If you can imagine it, then you can achieve it, but it might take a lot of time, it might take a lot of hard work. If you want something you’ve never had, you have to do something you’ve never done.",
        "Barbra Straisand",
        "Climb every mountain",
        R.drawable.imagen35,
        R.drawable.streisand
    ),
    Quote(
        35,
        "Código Jedi",
        "No hay emoción, hay paz.\n" +
                "No hay ignorancia, hay conocimiento. \n" +
                "No hay pasión, hay serenidad.\n" +
                "No hay caos, hay armonía.\n" +
                "No hay muerte, hay esperanza",
        "",
        "",
        R.drawable.imagen36,
        R.drawable.yoda
    ),
    Quote(
        36,
        "Better",
        "When I’m good, I’m very good, but when I’m bad, I’m better.",
        "Rebecca Ferguson",
        "",
        R.drawable.imagen37,
        R.drawable.rebeccaf
    ),
    Quote(
        37,
        "¡Supéralo!",
        "La vida puede ser dura y, sin embargo, nos compadecemos de nosotros mismos cuando no es justa\n" +
                "\n" +
                "\t- Se burlarán de ti.\n" +
                "\t- Te sentirás inseguro.\n" +
                "\t- Puede que no seas el mejor todo el tiempo.\n" +
                "\t- Puede que seas el único negro, blanco, asiático, latino, mujer, hombre, gay, lesbiana o [pon tu identidad aquí] en una situación determinada.\n" +
                "\t- Habrá momentos en los que te sentirás solo.\n" +
                "\n" +
                "¡Supéralo!",
        "David Goggins",
        "No me puedes dañar",
        R.drawable.imagen38,
        R.drawable.goggins
    ),
    Quote(
        38,
        "Desaprender lo aprendido",
        "El progreso requiere desaprender lo aprendido. Para lograr ser la mejor versión de ti mismo, es necesario que edites constantemente tus creencias y que actualices y expandas tu identidad.",
        "James Clear",
        "Hábitos atómicos",
        R.drawable.imagen39,
        R.drawable.clear
    ),
    Quote(
        39,
        "Convertirte en la persona que eres",
        "El proceso de construcción de hábitos equivale en realidad al proceso de convertirte en la persona que eres.",
        "James Clear",
        "Hábitos atómicos",
        R.drawable.imagen40,
        R.drawable.clear
    ),
    Quote(
        40,
        "Walls",
        "Brick walls are there for a reason. The brick walls are not there to keep us out. The brick walls are there to show how badly we want something. Because the brick walls are there to stop the people who don't want something badly enough.",
        "Randy Pausch",
        "The Last Lecture",
        R.drawable.imagen41,
        R.drawable.pausch
    ),
    Quote(
        41,
        "Triunfo del mal",
        "Lo único que necesita el mal para triunfar es que los hombres buenos no hagan nada",
        "Edmund Burk",
        "",
        R.drawable.imagen42,
        R.drawable.burke
    ),
    Quote(
        42,
        "Understand",
        "Any fool can know. The point is to understand.",
        "Albert Einstein",
        "",
        R.drawable.imagen43,
        R.drawable.einstein
    )
)

fun getRandomQuote(): Quote {
    return quotes.random()
}
















